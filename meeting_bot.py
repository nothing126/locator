...
class MeetingBot:
    ...
    def __init__(self, token):
        self.bot = telebot.TeleBot(token)  # инициализация переменной для работы с ботом
    ...
    # инициализация начальной команды (start)
    def start(self):
        ...
        def start(message):
            ...
            if member_count > 2:
                ...
                self.bot.send_message(chat_id,
                                      "Здравствуйте, этот бот поможет выбрать вам и вашим друзьям место встречи "
                                      "исходя "
                ...
                self.bot.send_message(chat_id, "спасибо за активацию, теперь вернитесь в группу и ")

        # функция для инициализации команды для начала сеанса
        @self.bot.message_handler(func=lambda message: message.text.lower() == '/meet')
        def meet(message):
            chat_id = message.chat.id
            member_count = self.bot.get_chat_member_count(chat_id)  # получение колличества участников

            if member_count > 2:  # Проверка, что это групповой чат
            ...
            if member_count <= 2:
                ...
                # предупреждение что только администриатор и создатель может ввести команду
        ...
        def confirm_meet_callback(call):
            user_id = call.from_user.id  # получение id пользователя
            chat_id = call.message.chat.id  # получение id чата
            username = call.message.from_user.username  # получение юзернейма
        ...
        def location(message):
            ...
            long = message.location.longitude  # долгота
            loc = f"{lat},{long}"  # широта, долгота
